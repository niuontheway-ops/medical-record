...
export default function Home(){
 ...
 <header className="masthead"><a href={`${basePath}/`} className="brand"><span className="brand-mark"><FileText size={23}/></span><span>临床第一课<small>病历书写入门</small></span></a><div className="publisher-brand" aria-label="壶兰呼吸，by 呼吸与危重症医学科老崔"><span className="publisher-mark" aria-hidden="true">壶</span><span><strong>壶兰呼吸</strong><small>by 呼吸与危重症医学科 · 老崔</small></span></div><a href="#references" className="reference-link">规范依据 <ExternalLink size={14}/></a></header>
 <main className="shell">
 <Tabs value={active} onValueChange={v=>navigate(String(v))} className="course-tabs">
 <section className="course-index" aria-labelledby="course-index-title"><div className="chapter-heading"><h2 id="course-index-title">学习目录</h2><span>点击章节直达内容</span></div><div className="chapter-scroll"><TabsList className="chapter-list" aria-label="选择病历书写课程">{lessons.map((l,i)=><TabsTrigger key={l.id} value={l.id} className="chapter-tab"><span className="chapter-number">{String(i+1).padStart(2,"0")}</span>{l.title}{answers[l.id]!==undefined&&<Check size={14} aria-label="已练习"/>}</TabsTrigger>)}</TabsList></div></section>
